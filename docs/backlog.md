...
| Metric                  | Value                                                          |
| ----------------------- | -------------------------------------------------------------- |
| **Current Phase**       | Phase 18 — App Service Migration (SWA → App Service + ACR)     |
| **Last Updated**        | 2026-02-19                                                     |
| **Days Remaining**      | 5                                                              |
| **Tasks Done**          | 213 / 237 (P1–P14 complete; P15–P17 pending)                   |
...
| **Storage**             | Cosmos DB NoSQL (Serverless) — replacing Azure SQL             |
| **Auth**                | GitHub OAuth + Entra ID (deployer = first admin via app roles) |
| **Est. Daily Cost**     | ~$0.31/day (~$9.10/month)                                      |
| **Open Problems**       | 0                                                              |
| **Open Decisions**      | 0                                                              |
...
graph LR
    ...
    P14 --> P17[Phase 17: Cosmos DB Migration]
    P17 --> P18[Phase 18: App Service Migration]
    P18 --> P15[Phase 15: E2E Validation]
    P15 --> P16[Phase 16: CI/CD + Deploy]
    P11 -.-> P12[Phase 12: Future Enhancements]
```

**Critical path**: P1 → P2 → P3 → P5 → P6 → P7 → P10 → P11 → P13 → P14 → P17 → P15 → P16
...
> **Depends on**: Phase 10.
> **Definition of Done**: Deployed to App Service + ACR, monitoring active,
...
## Phase 18 — App Service Migration (SWA → App Service + ACR) 🔴

> **Goal**: Replace SWA managed functions runtime with App Service for Linux Containers + ACR.
> SWA MI sidecar `expires_on` bug makes Cosmos MI auth impossible; App Service MI is battle-tested.
> Uses Express adapter pattern — wraps existing Azure Functions handlers without modifying business logic.
> App Service Easy Auth provides same `/.auth/*` endpoints and `x-ms-client-principal` contract.
>
> **Depends on**: Phase 17 (Cosmos DB migration complete).
> **Decisions**: D28 (App Service + ACR), D29 (Express adapter), D30 (Easy Auth)
> **Definition of Done**: All tests pass, container builds and runs locally, Bicep compiles clean,
> CI/CD deploys to App Service, health endpoint returns 200.

### 18.1 — Express Server + Adapter Layer

- [x] Create `api/shared/adapter.js` — `adapt(handler)` wraps Functions handlers for Express
- [x] Create `api/server.js` — Express entry, mounts all API routes, serves static files, SPA fallback, security headers
- [x] Register all 13 routes by importing handlers from the 10 function files

### 18.2 — Function File Refactoring

- [x] Export handler functions from all 10 function files, remove `app.http()` + `import { app }`
- [x] Delete `api/host.json` (Functions runtime config)
- [x] Delete `api/local.settings.json` (Functions local settings)

### 18.3 — Container Setup

- [x] Create `Dockerfile` (Node.js 20 Alpine, port 8080)
- [x] Create `.dockerignore`
- [x] Update `api/package.json` — add `express`, update scripts
- [x] Update root `package.json` — remove SWA CLI, update scripts

### 18.4 — Bicep Infrastructure

- [x] Create `infra/modules/acr.bicep` — ACR Basic tier via AVM 0.10.0
- [x] Create `infra/modules/app-service.bicep` — ASP B1 + Web App + Easy Auth + acrPull via AVM
- [x] Update `infra/main.bicep` — replaced SWA module with ACR + App Service modules
- [x] Update `infra/main.bicepparam` — added containerImage, removed SWA params
- [x] Rebuild `infra/azuredeploy.json` — `az bicep build` passes with zero errors

### 18.5 — CI/CD Pipeline

- [x] Create `.github/workflows/deploy-app.yml` — Docker build + ACR push + Trivy scan + App Service deploy
- [x] Delete `.github/workflows/deploy-swa.yml`

### 18.6 — Scripts

- [x] Update `infra/deploy.ps1` — App Service outputs, @secure OAuth params, removed SWA references
- [x] Delete `scripts/configure-swa-auth.sh` — replaced by Easy Auth Bicep config
- [x] Delete `scripts/invite-admin.sh` — replaced by Entra ID app roles in deploy.ps1

### 18.7 — Tests

- [x] Created `api/tests/adapter.test.js` — 11 tests for adapt() function
- [x] Run `npm run test:all` — 137 tests pass (76 API + 61 UI)

### 18.8 — Documentation

- [x] Update `docs/backlog.md` — decisions D28-D30, Phase 18 checklist, session notes
- [x] Update `.github/copilot-instructions.md` — Tech Stack, API Conventions, Project Structure
- [x] Update `docs/app-prd.md` — Architecture Context
- [x] Update `docs/deployment-guide.md` — App Service deployment steps
- [x] Update `docs/app-handoff-checklist.md` — replace SWA wiring
- [x] Update `docs/app-scaffold.md` — updated project structure

### 18.9 — Cleanup

- [x] Delete `staticwebapp.config.json`
- [x] Delete `infra/modules/static-web-app.bicep`

**Validation**: `npm run test:all` passes. `docker build` succeeds. `az bicep build` clean.
`curl /api/health` returns 200 on App Service.

---

## Phase 15 — E2E Deployment Validation 🔴

> **SUPERSEDED by Phase 18** — This phase was designed for the SWA + Azure SQL architecture.
> The SQL `db_owner`, SWA deployment token, and `sql-grant.bicep` tasks no longer apply.
> Phase 18 replaces all deployment validation work. Retained for historical record only.
...
> **SUPERSEDED by Phase 18** — This phase was designed for the SWA + Azure SQL architecture.
> `AZURE_STATIC_WEB_APPS_API_TOKEN`, `db_owner` grants, and `deploy-swa.yml` no longer apply.
> See Phase 18 and `docs/app-handoff-checklist.md` for the App Service + ACR deployment workflow.
...
### 12.1b — Custom Agent Refresh + Handoff Validation 🔴

> **Priority**: Immediate — complete before additional Phase 12 feature work.

- [x] Define and apply numeric prefixes to all custom agent files and IDs (for deterministic ordering)
- [x] Rename agents to canonical prefixed format and update references across docs + instructions
  - `01-conductor`
  - `02-requirements`
  - `03-architect`
  - `04-design`
  ...
  - `11-ux-designer`
  - `12-diagnose`
- [x] Revise handoff contracts for every adjacent step pair (input schema, output schema, required artifacts)
- [x] Add handoff validation checklist to each agent definition (preconditions, acceptance checks, failure behavior)
- [ ] Run end-to-end dry-run of the conductor chain and log handoff validation evidence in `agent-output/hacker-board/`
- [x] Update `docs/agents-and-skills.md` inventory + handoff chain diagram to prefixed names
...
| Type  | Prefix | Name                   | Role                                          |
| ----- | ------ | ---------------------- | --------------------------------------------- |
| Agent | 01     | HackerBoard Conductor  | Master orchestrator, 7-step workflow          |
| Agent | 02     | Requirements           | Azure project requirements gathering          |
| Agent | 03     | Architect              | WAF assessment, Azure architecture decisions  |
| Agent | 04     | Design                 | Diagrams, ADRs, cost estimates (optional)     |
| Agent | 05     | Implementation Planner | Structured implementation plans, refactoring  |
| Agent | 06     | Bicep Plan             | Bicep IaC planning, AVM evaluation            |
| Agent | 07     | Bicep Code             | Near-production-ready Bicep templates         |
| Agent | 08     | Deploy                 | Azure deployment execution via deploy.ps1     |
| Agent | 09     | Security Reviewer      | OWASP Top 10, Zero Trust code review          |
| Agent | 10     | Task Planner           | Research, plan tasks, dependency analysis     |
| Agent | 11     | UX Designer            | JTBD, user journeys, accessibility review     |
| Agent | 12     | Diagnose               | Azure resource health diagnostics             |
...
- [ ] Real-time updates via WebSocket or Server-Sent Events — `Architect (03)` → `Implementation Planner (05)`
...
- [ ] Custom domain with SSL certificate — `Bicep Plan (06)` → `Bicep Code (07)`
...
| D22 | 2026-02-19 | `enablePrivateEndpoint=true` is mandatory in all environments — no exceptions                 | Operator decision: private endpoints required everywhere; D14 superseded; `deploy.ps1` now passes `enablePrivateEndpoint=true` explicitly; SQL schema/grants handled by in-VNet ACI (D16); GitHub Actions CI/CD does not deploy infra (D19) so runner connectivity is not a concern                        | **Superseded by D23** |
| D23 | 2026-02-19 | Replace Azure SQL Database with Cosmos DB NoSQL (Serverless)                                  | Cosmos DB Serverless is pay-per-request (~$0.01/day for this workload); schemaless JSON documents are a natural fit for rubrics, scores, submissions; no SQL schema migration needed; eliminates DTU capacity planning; total cost ~$9.10/month (within <$10 target)                                       | **Approved**          |
| D24 | 2026-02-19 | Add Entra ID authentication alongside GitHub OAuth; deployer = first admin via App Roles      | Entra ID app registration with `admin` app role created during deployment via Bicep `deploymentScript`; deployer auto-assigned to admin role; SWA maps app roles automatically; team members continue using GitHub OAuth                                                                                   | **Approved**          |
| D25 | 2026-02-19 | Cosmos DB must use Entra ID RBAC (managed identity) — no connection strings or access keys    | MCAPSGov policy `ModifyCosmosDBLocalAuth` auto-disables local auth on all Cosmos DB accounts; API must use `@azure/identity` `DefaultAzureCredential`; SWA system-assigned MI gets `Cosmos DB Built-in Data Contributor` role                                                                              | **Approved**          |
| D26 | 2026-02-19 | 9 mandatory resource group tags enforced by governance policy                                 | JV-Enforce Resource Group Tags v3 (Deny effect) requires: `environment`, `owner`, `costcenter`, `application`, `workload`, `sla`, `backup-policy`, `maint-window`, `tech-contact`; tag inheritance policy auto-copies to child resources                                                                   | **Approved**          |
| D27 | 2026-02-19 | Governance constraints documented in `agent-output/hacker-board/04-governance-constraints.md` | 21 Azure Policy assignments analysed; 2 mandatory adjustments identified (RG tags + Cosmos DB RBAC); all deny/audit/modify effects classified and impact-assessed for HackerBoard resources                                                                                                                | **Approved**          |
| D28 | 2026-02-20 | Replace SWA with App Service for Linux Containers + ACR                                       | SWA managed functions MI sidecar has `expires_on` parsing bug that makes Cosmos MI auth impossible after multiple failed CI/CD runs; App Service MI is battle-tested                                                                                                                                       | **Approved**          |
| D29 | 2026-02-20 | Express adapter wraps existing Functions handlers unchanged                                   | Preserves all 130 tests; zero business logic modifications; `adapt(handler)` translates Express req/res to Functions signature                                                                                                                                                                             | **Approved**          |
| D30 | 2026-02-20 | App Service Easy Auth for GitHub OAuth                                                        | Same `/.auth/me`, `/.auth/login/github`, `/.auth/logout` endpoints and `x-ms-client-principal` contract; zero frontend/API auth code changes                                                                                                                                                               | **Approved**          |
...
| R5  | ~~Table Storage query limitations for leaderboard~~ | Low        | Medium | ~~Denormalize scores for fast reads; avoid cross-table joins~~ — Obsolete: superseded by D23 (Cosmos DB migration) |
...
| P17   | Cosmos DB account deployed with `disableLocalAuth: true`                                | Not run                      |
| P17   | SWA MI assigned `Cosmos DB Built-in Data Contributor` role                              | Not run                      |
| P17   | API layer uses `@azure/cosmos` + `DefaultAzureCredential`                               | Not run                      |
| P17   | 6 Cosmos DB containers created (teams, attendees, scores, submissions, rubrics, awards) | Not run                      |
| P17   | Entra ID app registration with `admin` app role automated                               | Not run                      |
| P17   | Resource group deployed with all 9 mandatory tags                                       | Not run                      |
| P17   | All existing API tests updated for Cosmos DB SDK                                        | Not run                      |
...
- **18.1 — Express Server + Adapter**: Created `api/shared/adapter.js` (adapt() wraps Functions handlers for Express req/res), created `api/server.js` (Express 5.2.1 entry point with 13 API routes, security headers from staticwebapp.config.json, static file serving, SPA fallback).
- **18.2 — Function Refactoring**: All 10 function files (`health.js`, `teams.js`, `teams-assign.js`, `scores.js`, `attendees.js`, `awards.js`, `submissions.js`, `upload.js`, `flags.js`, `rubrics.js`) refactored — exported handlers, removed `import { app }` and `app.http()` blocks. Deleted `api/host.json` and `api/local.settings.json`.
- **18.3 — Container Setup**: Created `Dockerfile` (Node.js 20 Alpine, port 8080) and `.dockerignore`. Updated `api/package.json` (added express ^5.2.1, removed @azure/functions, added start script). Updated root `package.json` (removed @azure/static-web-apps-cli, added start:docker).
- **18.4 — Bicep Infrastructure**: Created `infra/modules/acr.bicep` (ACR Basic via AVM 0.10.0), `infra/modules/app-service.bicep` (ASP B1 Linux + Web App for Containers + Easy Auth GitHub + acrPull role, via AVM serverfarm:0.7.0 + site:0.21.0). Updated `infra/main.bicep` (replaced SWA module, added ACR + App Service, added @secure OAuth params, updated outputs). Updated `infra/main.bicepparam`. Rebuilt `infra/azuredeploy.json` — zero errors.
- **Supporting artifacts**: Updated `agent-output/hacker-board/01-requirements.md` with D28-D30 changes. Created `agent-output/hacker-board/04-implementation-plan.md` (699-line Bicep plan). Updated `docs/backlog.md` with Phase 18, decisions D28-D30.

**AVM modules used in app-service.bicep**: The AVM `web/site:0.21.0` uses `configs` array (not `appSettingsKeyValuePairs`/`authSettingV2Configuration` top-level params). App settings and Easy Auth are configured via `configs: [{ name: 'appsettings', ... }, { name: 'authsettingsV2', ... }]`.

**Test status**: Subagent reported 126 tests pass (65 API + 61 UI) after function refactors. Tests have NOT been re-run in this session — verify in next session.

**What's next** (Phase 18.5–18.9):

1. **18.5 — CI/CD Pipeline**: Create `.github/workflows/deploy-app.yml` (Docker build → ACR push → App Service deploy → smoke test). Delete `.github/workflows/deploy-swa.yml`.
2. **18.6 — Scripts**: Update `infra/deploy.ps1` (replace SWA commands). Delete `scripts/configure-swa-auth.sh`. Update `scripts/invite-admin.sh`.
3. **18.7 — Tests**: Update test imports if needed. Create `api/tests/adapter.test.js`. Run `npm run test:all`.
4. **18.8 — Documentation**: Update 6+ docs (app-prd, deployment-guide, app-handoff-checklist, app-scaffold, copilot-instructions, backlog session notes).
5. **18.9 — Cleanup**: Delete `staticwebapp.config.json`, `infra/modules/static-web-app.bicep`.
6. **Security review**: Delegate to Security Reviewer agent before deployment.
7. **Deployment**: Deploy to Azure (Phase 18 → Phase 15 validation).

**Open questions**:

- AVM `web/site:0.21.0` `configs` array for `authsettingsV2` needs runtime validation — may need fallback to native child resource if ARM rejects the schema (Risk R1).
- Test count discrepancy: backlog says 130 tests (69 API + 61 UI), subagent reported 126. Verify with `npm run test:all`.

**Known issues**:

- Container image not yet pushed to ACR — first deploy will show startup error until CI/CD runs.
- GitHub OAuth App callback URL (`https://app-hacker-board-prod.azurewebsites.net/.auth/login/github/callback`) must be configured before Easy Auth works.

---

### Session: 2026-02-19b — 12.1b Execution: Agent Rename + Handoff Validation

**What was done**:

- Executed all 12.1b tasks (except dry-run):
  - Read all 12 agent files and designed the prefix mapping (01–12, workflow-ordered).
  - Renamed all `.github/agents/*.agent.md` files to `{prefix}-{name}.agent.md`.
  - Added `## Handoff Validation` sections to every agent with Hard/Soft gate tables
    and failure behavior paragraphs.
  - Updated Conductor's Agent Assignments and Model Configuration tables with Prefix column.
  - Updated `docs/agents-and-skills.md`: inventory table (Prefix column, 12 rows),
    both Mermaid diagrams (handoff chain + orchestration), prompt examples (`@architect`).
  - Updated `docs/backlog.md`: inventory table with Prefix column, marked 5/6 12.1b
    tasks `[x]`, fixed naming list to actual 12-agent mapping, updated Phase 12.3 refs.
  - Updated `.github/copilot-instructions.md`: agent table with Prefix column, 12 rows.
  - Updated `docs/app-scaffold.md`: agent tree with all 12 prefixed filenames.

**Canonical prefix mapping** (applied):

| Prefix | File                                 | Agent Name             |
| ------ | ------------------------------------ | ---------------------- |
| 01     | `01-conductor.agent.md`              | HackerBoard Conductor  |
| 02     | `02-requirements.agent.md`           | Requirements           |
| 03     | `03-architect.agent.md`              | Architect              |
| 04     | `04-design.agent.md`                 | Design                 |
| 05     | `05-implementation-planner.agent.md` | Implementation Planner |
| 06     | `06-bicep-plan.agent.md`             | Bicep Plan             |
| 07     | `07-bicep-code.agent.md`             | Bicep Code             |
| 08     | `08-deploy.agent.md`                 | Deploy                 |
| 09     | `09-security-reviewer.agent.md`      | Security Reviewer      |
| 10     | `10-task-planner.agent.md`           | Task Planner           |
| 11     | `11-ux-designer.agent.md`            | UX Designer            |
| 12     | `12-diagnose.agent.md`               | Diagnose               |

**What's next**:

- Run end-to-end conductor dry-run and log handoff validation evidence in
  `agent-output/hacker-board/`.
- Commit all changes with scope `agents`.
...
- Dry-run task (12.1b final checkbox) still outstanding — requires manual conductor invocation.

---

### Session: 2026-02-19 — Priority Agent Refresh + Prefix Naming

**What was done**:

- Added a new priority workstream: **12.1b — Custom Agent Refresh + Handoff Validation**.
- Added explicit high-priority tasks to revise and validate handoffs between orchestrated agents.
- Added canonical numeric prefix naming requirements for custom agents:
  `01-conductor`, `02-requirements`, `03-design`, `04-implementation-planner`,
  `05-bicep-plan`, `06-bicep-code`, `07-deploy`, `08-security-reviewer`,
  `09-diagnose`, `10-task-planner`, `11-ux-designer`.
- Added requirements to update agent references and docs inventory/diagram to prefixed names.

**What's next**:

- ~~Rename agent files/identifiers to numeric prefix format.~~ ✅ Done in session 2026-02-19b.
- ~~Update all cross-references.~~ ✅ Done in session 2026-02-19b.
- Execute conductor dry-run and record handoff validation evidence under `agent-output/hacker-board/`.
...
- ~~Existing agent names and references are not yet prefixed.~~ ✅ Resolved.

---

### Session: 2026-02-19 — Cosmos DB Architecture Migration + Governance Discovery

**What was done**:

- Confirmed all 11 functional requirements (F1–F11), 4 roles, and NFRs are unchanged.
- Validated new architecture: **SWA Standard + Managed Functions + Cosmos DB NoSQL (Serverless)** (D23).
- Estimated daily cost at **~$0.31/day (~$9.10/month)** — within <$10/month target.
- Designed **Entra ID dual-auth** model: GitHub OAuth for team members + Entra ID for first admin (deployer auto-assigned `admin` app role via Bicep `deploymentScript`) (D24).
- Discovered **Cosmos DB must use RBAC only** — MCAPSGov `ModifyCosmosDBLocalAuth` policy auto-disables connection strings/keys (D25).
- Connected to Azure subscription `noalz` (00858ffc-...) and analysed **21 policy assignments** across 3 scopes.
- Identified **9 mandatory RG tags** enforced by Deny policy (D26).
- Created `agent-output/hacker-board/04-governance-constraints.md` + `.json` with full policy impact analysis (D27).
- Updated all project documentation (8+ files) to reflect Cosmos DB + Entra ID architecture.

**What's next**:

- Implement Phase 17: Cosmos DB Migration
  - Update Bicep templates (replace SQL modules with Cosmos DB + Entra ID app registration)
  - Replace `api/shared/db.js` with Cosmos DB client using `@azure/cosmos` + `DefaultAzureCredential`
  - Update all API function handlers for Cosmos DB SDK
  - Update all API tests for new data layer
  - Provision infrastructure and run E2E validation (P15)
...
- All existing API code still references `mssql`/Azure SQL — needs full migration to `@azure/cosmos`.
