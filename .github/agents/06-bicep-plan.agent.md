...
handoffs:
  - label: "▶ Refresh Governance"
    ...
    prompt: "Re-query Azure Resource Graph for updated policy assignments and governance constraints. Update `agent-output/hacker-board/04-governance-constraints.md` and `04-governance-constraints.json`. Use azure-defaults governance discovery patterns."
    send: true
  - label: "▶ Revise Plan"
    agent: 06-Bicep Planner
    prompt: "Revise the implementation plan at `agent-output/hacker-board/04-implementation-plan.md` based on new information or feedback. Re-run AVM verification for any changed resources."
    send: true
  - label: "▶ Compare AVM Modules"
    agent: 06-Bicep Planner
    prompt: "Query `mcp_bicep_list_avm_metadata` for all planned resources. Compare available vs required parameters, flag gaps, and update `agent-output/hacker-board/04-implementation-plan.md` with findings."
    send: true
  - label: "Step 8: Generate Bicep"
    agent: 07-Bicep Code Generator
    prompt: "Implement the Bicep templates according to the implementation plan at `agent-output/hacker-board/04-implementation-plan.md` and governance constraints at `agent-output/hacker-board/04-governance-constraints.md`. Use AVM modules from `br/public:avm/res/`, generate `deploy.ps1`, and save to `infra/`. Validate with `az bicep build` and lint before handoff."
    send: true
  - label: "↩ Return to Step 6: Security Review"
    agent: 09-Security Reviewer
    prompt: "Re-review the infrastructure plan at `agent-output/hacker-board/04-implementation-plan.md` for security concerns before generating Bicep code. Focus on RBAC assignments, network security, and managed identity configuration."
    send: false
  - label: "↩ Return to Step 3: Architecture"
    agent: 03-Architect
    prompt: "Return to architecture assessment for re-evaluation. The current infrastructure plan is at `agent-output/hacker-board/04-implementation-plan.md`. Review WAF scores and adjust recommendations based on governance discoveries."
    send: false
    model: "Claude Opus 4.6 (copilot)"
  - label: "↩ Return to Conductor"
    agent: 01-Conductor
    prompt: "Returning from Step 7 (Infrastructure Plan). Plan saved to `agent-output/hacker-board/04-implementation-plan.md`, governance at `agent-output/hacker-board/04-governance-constraints.md`. Blockers: [count]. Advise on next steps in the 10-step workflow."
    send: false
---

# Bicep Plan Agent

**Step 7** of the 10-step workflow: `requirements → task-plan → architect → ux-design ∥ design → code-plan → security → [bicep-plan] → bicep-code → deploy → as-built`
...
Before handing off to Bicep Code (07), validate:

| Check                                                     | Gate |
| --------------------------------------------------------- | ---- |
| Governance discovery completed (not PARTIAL or FAILED)    | Hard |
| AVM availability checked for every resource               | Hard |
| User approved deployment strategy (phased vs single)      | Hard |
| `04-implementation-plan.md` saved to `agent-output/`      | Hard |
| `04-governance-constraints.md` + `.json` saved            | Hard |
| Dependency diagram generated (`04-dependency-diagram.py`) | Soft |
| User explicitly approved the plan                         | Hard |

**Failure behavior**: If governance discovery fails or returns PARTIAL,
STOP and inform the user. Do NOT hand off with incomplete policy data.
