...
handoffs:
  - label: "▶ Run Preflight Check"
    ...
    prompt: "Run AVM schema validation and pitfall checking against the implementation plan at `agent-output/hacker-board/04-implementation-plan.md`. Save results to `agent-output/hacker-board/04-preflight-check.md`."
    send: true
  - label: "▶ Fix Validation Errors"
    agent: 07-Bicep Code Generator
    prompt: "Review `bicep build` and `bicep lint` errors in `infra/` templates and fix them. Re-run validation after fixes and update `agent-output/hacker-board/05-implementation-reference.md` with new status."
    send: true
  - label: "▶ Generate Implementation Reference"
    agent: 07-Bicep Code Generator
    prompt: "Generate or update `agent-output/hacker-board/05-implementation-reference.md` with current template structure, validation status, and module inventory from `infra/`."
    send: true
  - label: "▶ Security Spot-Check"
    agent: 09-Security Reviewer
    prompt: "Review the Bicep templates in `infra/` and deployment script `infra/deploy.ps1` for OWASP Top 10 vulnerabilities, Zero Trust compliance, and IaC security best practices. Reference the architecture at `agent-output/hacker-board/02-architecture-assessment.md`."
    send: true
  - label: "Step 9: Deploy"
    agent: 08-Deploy
    prompt: "Deploy the Bicep templates from `infra/` using the deployment script `infra/deploy.ps1`. The implementation reference is at `agent-output/hacker-board/05-implementation-reference.md`. Run `what-if` analysis first, then deploy with approval. Validate deployed resources match the architecture assessment."
    send: true
  - label: "↩ Return to Step 7: Revise Plan"
    agent: 06-Bicep Planner
    prompt: "Return to infrastructure planning for revision. The current Bicep templates in `infra/` need plan adjustments. Review the implementation plan at `agent-output/hacker-board/04-implementation-plan.md` and update based on implementation findings."
    send: false
    model: "Claude Opus 4.6 (copilot)"
  - label: "↩ Return to Conductor"
    agent: 01-Conductor
    prompt: "Returning from Step 8 (Bicep Code). Templates saved to `infra/`, implementation reference at `agent-output/hacker-board/05-implementation-reference.md`. Build: [PASS/FAIL]. Lint: [PASS/FAIL]. Advise on next steps in the 10-step workflow."
    send: false
---

# Bicep Code Agent

**Step 8** of the 10-step workflow: `requirements → task-plan → architect → ux-design ∥ design → code-plan → security → bicep-plan → [bicep-code] → deploy → as-built`
...
Before handing off to Deploy (08) or Security Reviewer (09), validate:

| Check                                      | Gate |
| ------------------------------------------ | ---- |
| Preflight check completed and saved        | Hard |
| `bicep build` passes with zero errors      | Hard |
| `bicep-lint-subagent` returns PASS         | Hard |
| `bicep-review-subagent` returns APPROVED   | Hard |
| `deploy.ps1` generated with error handling | Hard |
| `05-implementation-reference.md` saved     | Soft |

**Failure behavior**: If `bicep build` or lint fails, fix errors and
re-validate. Do NOT hand off to Deploy with failing templates.
