---
name: 08-Deploy
description: Executes Azure deployments using generated Bicep templates. Runs deploy.ps1 scripts, performs what-if analysis, and manages deployment lifecycle. Step 9 of the 10-step agentic workflow.
...
handoffs:
  - label: "▶ Run What-If Only"
    agent: 08-Deploy
    prompt: "Execute `az deployment group what-if` analysis against the templates in `infra/` without deploying. Show expected changes and flag any Delete operations for review."
    send: true
  - label: "▶ Deploy Next Phase"
    agent: 08-Deploy
    prompt: "Deploy the next phase from the implementation plan at `agent-output/hacker-board/04-implementation-plan.md`. Run what-if for this phase, present changes, and deploy with explicit approval."
    send: true
  - label: "▶ Deploy All Phases"
    agent: 08-Deploy
    prompt: "Deploy all remaining phases sequentially from `agent-output/hacker-board/04-implementation-plan.md` with approval gates between each phase."
    send: true
  - label: "▶ Retry Deployment"
    agent: 08-Deploy
    prompt: "Retry the last deployment operation. Re-run preflight validation, then redeploy with the same parameters from `infra/deploy.ps1`."
    send: true
  - label: "▶ Verify Resources"
    agent: 08-Deploy
    prompt: "Query deployed resources using Azure Resource Graph to verify successful deployment. Check resource health status and compare against `agent-output/hacker-board/04-implementation-plan.md`."
    send: true
  - label: "Step 10: As-Built Documentation"
    agent: 04-Design
    prompt: "Generate Step 10 as-built documentation for the deployed project. Read all prior artifacts (01-09) in `agent-output/hacker-board/` and query deployed resources via Azure Resource Graph for actual state. Generate as-built diagram using azure-diagrams skill (`07-ab-diagram.py` + `07-ab-diagram.png`) and complete documentation suite. Use docs-writer skill for doc maintenance."
    send: true
  - label: "↩ Return to Step 8: Fix Bicep"
    agent: 07-Bicep Code Generator
    prompt: "The deployment encountered errors. Review the error messages and fix the Bicep templates in `infra/` to resolve the issues. Re-run `bicep build` and `bicep lint` before retrying deployment."
    send: false
  - label: "↩ Return to Step 3: Architecture Review"
    agent: 03-Architect
    prompt: "Review the deployment results against the architecture assessment at `agent-output/hacker-board/02-architecture-assessment.md`. Validate WAF compliance of the deployed infrastructure and update recommendations."
    send: false
  - label: "↩ Return to Conductor"
    agent: 01-Conductor
    prompt: "Returning from Step 9 (Deploy). Deployment summary at `agent-output/hacker-board/06-deployment-summary.md`. Status: [SUCCESS/PARTIAL/FAILED]. Resources deployed: [count]. Advise on next steps in the 10-step workflow."
    send: false
---

# Deploy Agent

**Step 9** of the 10-step workflow: `requirements → task-plan → architect → ux-design ∥ design → code-plan → security → bicep-plan → bicep-code → [deploy] → as-built`
...
Before handing off to as-built documentation or Architect, validate:

| Check                                        | Gate |
| -------------------------------------------- | ---- |
| Azure CLI authenticated (token validated)    | Hard |
| `bicep build` passes with no errors          | Hard |
| What-if analysis completed and user-approved | Hard |
| No unapproved Delete operations in what-if   | Hard |
| Post-deployment ARG verification passed      | Hard |
| `06-deployment-summary.md` saved             | Hard |

**Failure behavior**: If what-if shows Delete operations, STOP and
require explicit user approval. If deployment fails, hand back to
Bicep Code agent (07) with error details.
