...
> Generated by bicep-code agent | 2025-07-15 · Updated 2026-02-21 (Phase 18.4: VNet + Private Endpoint)
...
```text
infra/
├── main.bicep              # Main orchestration template (18 params, 8 modules, 8 outputs)
├── main.bicepparam         # Parameter file for prod environment
├── deploy.ps1              # PowerShell deployment script with post-deploy verification
...
└── modules/
    ...
    ├── cosmos-account.bicep           # AVM: document-db/database-account:0.18.0 (publicNetworkAccess: Disabled)
    ├── networking.bicep               # AVM: network/virtual-network:0.7.2 (VNet + snet-pe + snet-app)
    ├── cosmos-private-endpoint.bicep  # AVM: network/private-endpoint:0.11.1 + private-dns-zone:0.8.0
    ├── acr.bicep                      # AVM: container-registry/registry:0.10.0
    ├── app-service.bicep              # AVM: web/serverfarm:0.7.0 + web/site:0.21.0 + VNet integration
    └── cosmos-rbac.bicep              # Native Bicep: SQL role assignment (Data Contributor → App MI)
```
...
| `bicep lint`  | ✅     | Zero warnings                                      |
| IDE errors    | ✅     | Zero diagnostics in main.bicep and main.bicepparam |
| All tests     | ✅     | 137 tests pass (76 API + 61 UI)                    |
...
| Virtual Network + 2 Subnets       | `br/public:avm/res/network/virtual-network:0.7.2`                     | networking.bicep              |
| Private DNS Zone                  | `br/public:avm/res/network/private-dns-zone:0.8.0`                    | cosmos-private-endpoint.bicep |
| Private Endpoint (Cosmos DB)      | `br/public:avm/res/network/private-endpoint:0.11.1`                   | cosmos-private-endpoint.bicep |
| Container Registry (Standard)     | `br/public:avm/res/container-registry/registry:0.10.0`                | acr.bicep                     |
| App Service Plan (P1v3 Linux)     | `br/public:avm/res/web/serverfarm:0.7.0`                              | app-service.bicep             |
...
graph TD
    ...
    MAIN --> PE["cosmos-private-endpoint.bicep"]
    MAIN --> ACR["acr.bicep"]
    MAIN --> APP["app-service.bicep"]
    MAIN --> RBAC["cosmos-rbac.bicep"]
    LA --> R1["📊 law-hacker-board-prod"]
    AI --> R2["📈 appi-hacker-board-prod"]
    COSMOS --> R3["💾 cosmos-hacker-board-prod"]
    NET --> R7["🌐 vnet-hacker-board-prod"]
    PE --> R8["🔒 pep-cosmos-hacker-board-prod"]
    PE --> R9["🌐 privatelink.documents.azure.com"]
    ACR --> R4["📦 crhackerboardprod"]
    APP --> R5["🌐 app-hacker-board-prod"]
    APP --> R5b["⚙️ asp-hacker-board-prod"]
    RBAC --> R6["🔑 Cosmos SQL Role Assignment"]
    AI -.->|workspaceResourceId| LA
    APP -.->|cosmosEndpoint| COSMOS
    APP -.->|appInsightsConnectionString| AI
    APP -.->|acrLoginServer| ACR
    APP -.->|vnetSubnetId| NET
    PE -.->|subnetId| NET
    PE -.->|cosmosAccountId| COSMOS
    PE -.->|vnetId| NET
...
| D28: SWA → App Service + ACR                    | SWA MI sidecar `expires_on` bug                      | acr.bicep, app-service.bicep              |
...
| System MI on App Service → Cosmos RBAC          | Data Contributor role via deterministic GUID         | cosmos-rbac.bicep                         |
| System MI on App Service → ACR Pull             | `acrPull` built-in role                              | app-service.bicep                         |
...
| AVM modules for all primary resources           | 9 of 11 resources use AVM; 2 native role assignments | modules/                                  |
...
| ---------------------------- | --------------------------- | ------------------------------------ |
| B1: MFA on write operations  | ✅ Deployer responsibility  | N/A (Azure AD enforced)              |
| B3: 9 RG tags required       | ✅ deploy.ps1 sets all 9    | deploy.ps1                           |
| B5: MCAPSGov deny on classic | ✅ No classic resources     | All resources use ARM/AVM            |
| B6: Storage key auth deny    | ✅ No deployment scripts    | D27: Entra via deploy.ps1            |
| B7: Cosmos private endpoint  | ✅ VNet + PE + Private DNS  | networking + cosmos-private-endpoint |
| ModifyCosmosDBLocalAuth      | ✅ `disableLocalAuth: true` | cosmos-account.bicep                 |
| Cosmos publicNetworkAccess   | ✅ `Disabled`               | cosmos-account.bicep                 |
...
### D27 Decision: Deployment Scripts → deploy.ps1
