...
const mockRubric = {
  ...
  categories: [
    ...
    },
    {
      ...
      maxPoints: 50,
      criteria: [
        { name: "Backup", points: 25 },
        { name: "Monitoring", points: 25 },
      ],
    },
  ],
...
describe("ScoreSubmission", () => {
  ...
  it("renders dynamic form from active rubric", async () => {
    ...
    api.teams.list.mockResolvedValue([{ rowKey: "t1", name: "Team Alpha" }]);
  ...
  it("renders team dropdown with available teams", async () => {
    ...
    api.teams.list.mockResolvedValue([
      { rowKey: "t1", name: "Team Alpha" },
      { rowKey: "t2", name: "Team Beta" },
    ]);
